...
def data_prep_pipeline(
    ...
    if data_prep_mode == "training":
        ...
        feature_engineer = hdb_est.data_prep.feature_engineering.FeatureEngineer(
            params=config["data_prep"]["feature_engineering"]
        )

        # Engineer features
        logger.info("Generating mppa derived features...")
        with hdb_est.utils.timer("mppa derived feature generation"):
            derived_data_hdb = feature_engineer.engineer_features(
                clean_hdb_data=clean_hdb_data
            )
