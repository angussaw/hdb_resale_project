...
def main():
    ...
    if submit:
        ...
        if len(messages) > 0:
            ...
            with right:
                st.subheader(f'Distances to nearest amenity')
            ...
            with right:
                shap_values = requests.post(url = "http://127.0.0.1:8500/explain",  data = json.dumps(derived_input_data.json(), default=str))
                if shap_values.json():
